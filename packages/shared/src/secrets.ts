...
let cachedUserSecret: string | null = null;
 ...
 */
export async function _getUserSecret(): Promise<string> {
  ...
  if (result[USER_SECRET_KEY]) {
    cachedUserSecret = result[USER_SECRET_KEY] as string;
    return cachedUserSecret;
  }

  const secret = generateUserSecret();
  await storage.set({ [USER_SECRET_KEY]: secret });
  cachedUserSecret = secret;
  return cachedUserSecret;
}

export function getUserSecret(): Promise<string> {
  return (secretPromise ??= _getUserSecret());
}

export async function clearUserSecret() {
  const storage = setupStorageApi();
  await storage.set({ [USER_SECRET_KEY]: undefined });
  cachedUserSecret = null;
  secretPromise = null;
}
